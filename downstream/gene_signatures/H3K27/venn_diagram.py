...
for pathway in pathways:
    path_A = os.path.join(bed_dir, f"H3K27_A_{pathway}_genes_with_peaks.bed")
    path_N = os.path.join(bed_dir, f"H3K27_N_{pathway}_genes_with_peaks.bed")
    ...
    if os.path.exists(path_A) and os.path.exists(path_N):
        ...
        plt.title(f"Peak Overlap in H3K27 (Pathway: {pathway.replace('_', ' ')})")

        out_file = f"venn_H3K27_{pathway}.png"
