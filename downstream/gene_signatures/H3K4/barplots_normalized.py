...
bed_dir = "/home/me1117/cutrun/gene_signatures/H3K4"  

# Define pathway pairs
pathways = ["eff", "il2", "il6", "mem", "tgfb", "tcr"]
...
plt.bar(x - bar_width, normalized_counts_A, width=bar_width, label="H3K4_A (Unique)", color="blue", alpha=0.7)
plt.bar(x, normalized_counts_N, width=bar_width, label="H3K4_N (Unique)", color="red", alpha=0.7)
...
plt.title("Normalized Unique Gene-Peak Intersections in H3K4_A vs H3K4_N")
plt.legend()
plt.grid(axis="y", linestyle="--", alpha=0.6)

# Save plot
plt.savefig("normalized_gene_peak_intersections_unique.png", dpi=300)
plt.show()
...
for file_A, file_N, pathway in file_pairs:
    ...
    if os.path.exists(path_A) and os.path.exists(path_N):
        ...
        count_A = len(genes_A) / total_peaks_A  # Normalize by total peaks in A
        count_N = len(genes_N) / total_peaks_N  # Normalize by total peaks in N
        count_common = len(common_genes) / ((total_peaks_A + total_peaks_N) / 2)  # Normalize by avg peaks
...
plt.bar(x - bar_width, normalized_counts_A, width=bar_width, label="H3K4_A", color="blue", alpha=0.7)
plt.bar(x, normalized_counts_N, width=bar_width, label="H3K4_N", color="red", alpha=0.7)
plt.bar(x + bar_width, normalized_counts_common, width=bar_width, label="Common", color="green", alpha=0.7)

plt.xticks(x, labels)  # Add pathway names on x-axis
plt.xlabel("Pathways")
plt.ylabel("Normalized Gene-Peak Intersections")
plt.title("Normalized Gene-Peak Intersections in H3K4_A vs H3K4_N")
plt.legend()
plt.grid(axis="y", linestyle="--", alpha=0.6)

# Save plot
plt.savefig("normalized_gene_peak_intersections_common.png", dpi=300)
plt.show()