...
file_pairs = [
    (
        f"H3K4_A_{p}_peaks_gene_intersections.bed",
        f"H3K4_N_{p}_peaks_gene_intersections.bed",
        p
    ) for p in pathways
]
...
plt.bar(x - bar_width / 2, proportions_A, width=bar_width, label="H3K4_A", color="blue", alpha=0.7)
plt.bar(x + bar_width / 2, proportions_N, width=bar_width, label="H3K4_N", color="red", alpha=0.7)

plt.xticks(x, labels)
plt.xlabel("Pathways")
plt.ylabel("Proportion of Genes with Peaks")
plt.title("Proportional Enrichment of H3K4me3 Peaks by Pathway (Gene-Centered)")
plt.legend()
plt.grid(axis="y", linestyle="--", alpha=0.6)

plt.tight_layout()
plt.savefig("proportional_genes_with_peaks_H3K4.png", dpi=300)
plt.show()